...
public class NormalCookBookFragment extends MVPBaseFragment<INormalCookBookFragment, NormalCookBookPresenter> implements INormalCookBookFragment {
    ...
    private void initView() {
        ...
        recyclerview.setHasFixedSize(true);     //设置recyclerview的高度为一致，以提高recyclerview的性能
        ...
        recyclerview.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
             //   Log.i("qin", "dx=" + dx + ",dy=" + dy);
                if (dy > 0) {       //如果recyclerview是向上滑动的，则计算是否已经滑动到recyclerview的底部
                    childCount = linearLayoutManager.getChildCount();   //得到当前界面显示的item数
                    totalCount = linearLayoutManager.getItemCount();      //获得所有的recyclerview包含的recyclerview的条数
                    lastCount = linearLayoutManager.findFirstVisibleItemPosition(); //获得在当前显示的recyclerview上面的item的条数
//                    Log.i("qin", "childCount" + childCount);
//                    Log.i("qin", "totalCount" + totalCount);
//                    Log.i("qin", "lastCount" + lastCount);

                    if (!mPresenter.isLoading && (childCount + lastCount) >= totalCount) {      //如果当前显示的recyclerview的条数加上在显示的item上面的item的条数
                        //大于等于总的item的条数，则证明已经滑动到底部，则开始刷新
                        mPresenter.getDate();
//                        Toast.makeText(mContext, "加载下一页", Toast.LENGTH_SHORT).show();
    ...
    public String getHomePageURL() {
        return "http://www.chinacaipu.com/menu/jiachangcaipu/";
    }
}
